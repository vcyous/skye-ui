...
import * as automationService from "../../services/api.js";
...
const AutomationsPage = () => {
  ...
  const [selectedWorkflowForStats, setSelectedWorkflowForStats] =
    useState(null);
  ...
  const handleCreateWorkflow = async (values) => {
    try {
      if (editingWorkflow) {
        await automationService.updateAutomationWorkflow(
          store.id,
          editingWorkflow.id,
          {
            name: values.name,
  ...
  const handleToggleWorkflow = async (workflowId, currentState) => {
    try {
      await automationService.toggleWorkflowEnabled(
        store.id,
        workflowId,
        !currentState,
      );
  ...
  const handlePauseWorkflow = async (workflowId, currentState) => {
    try {
      await automationService.pauseWorkflow(
        store.id,
        workflowId,
        !currentState,
      );
  ...
  const handleEditWorkflow = async (workflow) => {
    try {
      const fullWorkflow = await automationService.getAutomationWorkflowById(
        store.id,
        workflow.id,
      );
  ...
  const workflowColumns = [
    ...
    },
    {
      ...
      key: "triggerType",
      render: (triggerType) => {
        const template = triggerTemplates.find(
      ...
      render: (_, record) => {
        const rate =
          record.totalExecutions > 0
            ? (record.successfulExecutions / record.totalExecutions) * 100
            : 100;
  ...
  const loadExecutions = async (workflowId = null) => {
    setLoading(true);
    try {
      const data = await automationService.getAutomationExecutions(
        store.id,
        workflowId,
      );
      setExecutions(data);
  ...
  return (
    <div style={{ padding: "24px" }}>
      <div style={{ marginBottom: "24px" }}>
        ...
        <p>
          Reduce manual operations with safe automations and event-driven
          workflows.
        </p>
      </div>
        ...
        {selectedExecution && (
          <div>
            ...
            {selectedExecution.executionLogs &&
              selectedExecution.executionLogs.length > 0 && (
                <Card
                  size="small"
