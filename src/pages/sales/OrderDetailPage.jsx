...
export default function OrderDetailPage() {
  ...
  async function onCreateShipment(values) {
    ...
    setIsShipping(true);
    try {
      const unshipped = fulfillmentItems.filter(
              ...
              {[
                {
                  label: "Subtotal",
                  value:
                    order?.displaySubtotalAmount ?? order?.subtotalAmount ?? 0,
                },
                {
                  label: "Shipping",
                  value:
                    order?.displayShippingAmount ?? order?.shippingAmount ?? 0,
                },
                {
                  label: "Discount",
                  value: -(
            ...
            {order?.shippingAddress ? (
              <Space direction="vertical" size={2}>
                ...
                {Boolean(order.shippingAddress.addressLine2) && (
                  <Typography.Text style={{ fontSize: 13 }}>
                    {String(order.shippingAddress.addressLine2)}
                  </Typography.Text>
                )}
                <Typography.Text style={{ fontSize: 13 }}>
                  {[
