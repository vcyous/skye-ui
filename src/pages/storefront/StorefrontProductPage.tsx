...
interface StorefrontProductDetail extends PreviewProduct {
  description: string;
}
...
async function fetchProductByHandle(
  ...
  const variant = Array.isArray((data as any).product_variants)
    ? (data as any).product_variants[0]
    : null;
  return {
    id: (data as any).id,
    ...
    imageUrl:
      Array.isArray((data as any).media_urls) &&
      (data as any).media_urls.length > 0
        ? (data as any).media_urls[0]
        : null,
