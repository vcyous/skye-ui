...
import { getAuthFailureHandler } from "./authTokenUtils";
import {
  ensureAppUser,
  ensurePrimaryStore,
  getCurrentAuthUser,
...
import { AuthSessionData, UserProfile } from "../types";

export async function registerRequest(payload: any): Promise<AuthSessionData> {
  ...
  return {
    ...
    stores: store ? [mapStoreSummary(store)].filter(Boolean) as any : [],
    currentRole: null,
...
export async function loginRequest(payload: any): Promise<AuthSessionData> {
  assertSupabaseConfigured();
...
export async function refreshAccessToken(): Promise<AuthSessionData> {
  assertSupabaseConfigured();
...
export async function resetPasswordRequest(email: string): Promise<{ ok: boolean }> {
  assertSupabaseConfigured();
