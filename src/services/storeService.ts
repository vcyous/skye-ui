...
function normalizeCustomerTags(value: any): string[] {
...
function normalizeArrayValue(value: any): string[] {
  if (Array.isArray(value)) {
...
interface CustomerSegmentCondition {
  field: string;
  operator: string;
  value: any;
}
...
function normalizeCustomerSegmentFilter(input: any): CustomerSegmentFilter {
...
function resolveCustomerSegmentFieldValue(customer: any, field: string): any {
...
export async function resolveCollectionMembershipTable(): Promise<string> {
...
export async function resolveInventoryMovementTable(): Promise<string> {
...
export function normalizeInventoryReasonCode(value: any): string {
...
export function mapPublicUser(authUser: any, profile: any): UserProfile {
  return {
    id: authUser.id,
    name:
      profile?.full_name ||
...
export function mapStoreSummary(store: any): StoreSummary | null {
  if (!store) {
    return null;
  }
...
export function createRmaNumber(): string {
  return `RMA-${Date.now().toString(36).toUpperCase()}`;
}

export async function getCurrentAuthUser(): Promise<any> {
...
async function getAppUserById(userId: string): Promise<any> {
...
function resolveProfileName(authUser: any, payload: any = {}): string {
  return (
    payload.name ||
    payload.full_name ||
    authUser.user_metadata?.name ||
...
function resolveProfilePhone(authUser: any, payload: any = {}): string | null {
  return (
    payload.phone ||
...
async function insertAppUser(insertPayload: any): Promise<void> {
...
async function findPrimaryStoreByOwner(userId: string): Promise<any> {
  const { data, error } = await supabase
    .from("stores")
    .select("*")
    .eq("user_id", userId)
...
async function ensureThemes(storeId: string): Promise<void> {
  const { data, error } = await supabase
    .from("themes")
    .select("id")
    .eq("store_id", storeId)
...
export async function getStoreProfile(): Promise<any> {
...
export async function updateStoreProfile(payload: any): Promise<any> {
...
export async function updateStoreBranding(payload: any): Promise<any> {
  const { store } = await getStoreContext();
  const { data: themes, error: themeError } = await supabase
    .from("themes")
