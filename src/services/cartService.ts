...
export async function ensureActiveCart(storeId: string): Promise<DbCart> {
  ...
  return inserted as DbCart;
}

export async function getCart(): Promise<CartDetails> {
...
export async function addToCart(payload: { variantId: string; quantity?: number }): Promise<CartDetails> {
  const { store } = await getStoreContext();
  const cart = await ensureActiveCart(store.id);
...
export async function updateCartItemQuantity(cartItemId: string, quantity: number): Promise<CartDetails> {
  const { store } = await getStoreContext();
  ...
  if (nextQuantity > Number((item as any).product_variants?.quantity_in_stock || 0)) {
    throw new Error("Requested quantity exceeds current stock");
  }
...
export async function removeCartItem(cartItemId: string): Promise<CartDetails> {
  const { store } = await getStoreContext();
