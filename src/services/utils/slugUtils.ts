...
export function buildUniqueHandle(base: string): string {
  const normalized = slugify(base) || "item";
  return `${normalized}-${Date.now().toString(36)}`;
}

export function normalizeSeoHandle(value: string | null | undefined, fallback?: string | null): string {
...
export function validateSeoMetadataFields(payload: any = {}): void {
