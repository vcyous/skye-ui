...
export async function updateCompanyLocation(locationId, payload = {}) {
  ...
  const updates = {
    ...
    email: payload.email || null,
    is_primary:
      payload.isPrimary !== undefined ? Boolean(payload.isPrimary) : undefined,
    is_billing_address:
      payload.isBillingAddress !== undefined
        ? Boolean(payload.isBillingAddress)
        : undefined,
    is_shipping_address:
      payload.isShippingAddress !== undefined
        ? Boolean(payload.isShippingAddress)
        : undefined,
    updated_at: new Date().toISOString(),
  };
...
export async function updateCompanyBuyer(buyerId, payload = {}) {
  ...
  const updates = {
    ...
    role_id: payload.roleId || null,
    is_admin:
      payload.isAdmin !== undefined ? Boolean(payload.isAdmin) : undefined,
    status: payload.status || undefined,
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      estimated_volume: payload.estimatedVolume
        ? Number(payload.estimatedVolume)
        : null,
      status: payload.status || "active",
      metadata_json: payload.metadata || {},
    })
